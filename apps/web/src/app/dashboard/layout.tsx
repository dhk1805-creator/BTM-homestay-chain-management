...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden" style={{ background: '#080C16' }}>
      ...
      {/* Mobile top bar */}
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 lg:hidden" style={{ background: '#0D1220', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <button onClick={() => setMenuOpen(!menuOpen)} className="p-2 rounded-lg" style={{ color: '#E2E8F0' }}>
          {menuOpen ? (
        ...
        <button onClick={logout} className="p-2 rounded-lg" style={{ color: '#3D5A80' }}>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 lg:hidden" onClick={() => setMenuOpen(false)}>
          ...
          <div className="absolute top-14 left-0 bottom-0 w-72 overflow-auto" style={{ background: '#0D1220' }} onClick={e => e.stopPropagation()}>
            <nav className="px-3 py-4 space-y-1">
              {navItems.map((item) => {
                const isActive = pathname === item.href || (item.href !== '/dashboard' && pathname.startsWith(item.href));
                return (
                  <a key={item.href} href={item.href}
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isActive ? '' : 'hover:bg-white/[0.04]'}`}
                    style={isActive ? { background: 'rgba(56,138,221,0.12)', border: '1px solid rgba(56,138,221,0.2)' } : { border: '1px solid transparent' }}>
                    <span className="text-xl">{item.icon}</span>
                    <span className={`text-sm font-semibold ${isActive ? 'text-white' : ''}`} style={!isActive ? { color: '#4B6A8F' } : {}}>
                      {item.label}
                    </span>
                  </a>
                );
            ...
            {/* User info */}
            <div className="p-4 mx-3 mb-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      <aside className="w-64 flex-col flex-shrink-0 relative hidden lg:flex" style={{ background: '#0D1220' }}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <a key={item.href} href={item.href}
                ...
                <span className={`text-base font-semibold ${isActive ? 'text-white' : ''}`} style={!isActive ? { color: '#4B6A8F' } : {}}>
                  {item.label}
                </span>
      ...
      {/* Main content */}
      <main className="flex-1 overflow-auto pt-14 lg:pt-0" style={{ background: '#080C16' }}>
        {children}
      </main>
    </div>
  );
